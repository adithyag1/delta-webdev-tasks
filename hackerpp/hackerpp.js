...
let eaten=0;
let time=151;
let html,html1,html_spawn=``,html_portal,html_obstacle;
let msg;
let speed_time=200;
let board_size;
let obstaclex,obstacley;
let obstacle_direction=-1;
let spawnx, spawny;
let playing=false;
...
sizeform.addEventListener("submit",function(event){
    ...
    obstaclex = Math.floor(board_size/2);
    obstacley = board_size-3;
}
)
...
const move_obstacle = () => {
    if(obstacley===3) obstacle_direction=1;
    if(obstacley===board_size-3) obstacle_direction=-1;
    obstacley+=obstacle_direction;
    if(!(headx===obstaclex&&heady===obstacley+obstacle_direction)){
        html_obstacle=`<div class="obstacle" style="grid-area:${obstacley}/${obstaclex};"></div>`
    }
}
...
const portal = () => {
    ...
    while(portals.length<2){
        let portalx = Math.floor(Math.random() * 18)+2;
        let portaly = Math.floor(Math.random() * 18)+2;
        if(!(contains(foods_positions,[portalx,portaly])||contains(portals,[portalx,portaly])||(spawnx===portalx&&spawny===portaly))) {
            portals.push([portalx, portaly]);
            html_portal+=`<div class="portal" style="grid-area: ${portaly}/${portalx};"></div>`;
        }
    }
}
...
const start_game = () => {
    ...
    if(headx===obstaclex&&heady===obstacley){
        msg="You hit the obstacle";
        return updatelives();
    }
    for(let i=0;i<snake.length;i++){
        if(snake[i][0]===obstaclex&&snake[i][1]===obstacley){
            msg="You hit the obstacle";
            return updatelives();
        }
    }
    ...
    if(spawn_times.includes(time)){
        ...
        }while(contains(foods_positions,[spawnx,spawny]))
    ...
    for(let i=0;i<2;i++){
        if(headx===portals[i][0]&&heady===portals[i][1]){
            headx=portals[1-i][0];
            heady=portals[1-i][1];
            break;
            //if snake goes to one portal say 0, i=0 comes out from i=1-0=1 other portal
        }
    }

    move_obstacle();
    
    board.innerHTML = html+html1+html_spawn+html_portal+html_obstacle;
